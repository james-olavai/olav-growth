    ...
    for idx, info in enumerate(image_infos, 1):
        if hasattr(info, 'todict'):
            ...
            images_list.append({
                ...
                'urls': [d.get('download_url', '')] if d.get('download_url') else d.get('candidate_download_urls', []),
                'file_path': d.get('save_path', ''),
            ...
            images_list.append({
                ...
                'urls': info.get('candidate_urls', [info.get('download_url', '')]),
                'file_path': info.get('file_path', info.get('save_path', '')),
            ...
            images_list.append({
                ...
                'urls': [getattr(info, 'download_url', '')],
                'file_path': getattr(info, 'save_path', ''),
...
def search_platform(platform, keyword, num_images, output_dir, num_threads, save_meta):
    ...
    try:
        ...
        client = imagedl.ImageClient(
            image_sources=platform_client_name,
            init_image_clients_cfg={platform_client_name: {'work_dir': platform_dir}},
            clients_threadings={platform_client_name: num_threads}
        )

        print(f"[1/2] 正在搜索...")
        search_results = client.search(
            keyword,
            search_limits_per_source={platform_client_name: search_limits}
        )
        image_infos = search_results.get(platform_client_name, []) if isinstance(search_results, dict) else (search_results or [])
        ...
        metadata_serializable = [info.todict() if hasattr(info, 'todict') else info for info in image_infos]
